...
8;a<this.moduleCount-8;a++)null==this.modules[a][6]&&(this.modules[a][6]=0==a%2);for(a=8;a<this.moduleCount-8;a++)null==this.modules[6][a]&&(this.modules[6][a]=0==a%2)},setupPositionAdjustPattern:function(){for(var a=QRUtil.getPatternPosition(this.typeNumber),b=0;b<a.length;b++)for(var c=0;c<a.length;c++){var d=a[b],e=a[c];if(null==this.modules[d][e])for(var g=-2;2>=g;g++)for(var k=-2;2>=k;k++)this.modules[d+g][e+k]=-2==g||2==g||-2==k||2==k||0==g&&0==k?!0:!1}},setupTypeNumber:function(a){for(var b=
...
c-1]=d:9>c?this.modules[8][15-c-1+1]=d:this.modules[8][15-c-1]=d;this.modules[this.moduleCount-8][8]=!a},mapData:function(a,b){for(var c=-1,d=this.moduleCount-1,e=7,g=0,k=this.moduleCount-1;0<k;k-=2)for(6==k&&k--;;){for(var f=0;2>f;f++)if(null==this.modules[d][k-f]){var l=!1;g<a.length&&(l=1==(a[g]>>>e&1));QRUtil.getMask(b,d,k-f)&&(l=!l);this.modules[d][k-f]=l;e--; -1==e&&(g++,e=7)}d+=c;if(0>d||this.moduleCount<=d){d-=c;c=-c;break}}}};QRCode.PAD0=236;QRCode.PAD1=17;
QRCode.createData=function(a,b,c){b=QRRSBlock.getRSBlocks(a,b);for(var d=new QRBitBuffer,e=0;e<c.length;e++){var g=c[e];d.put(g.mode,4);d.put(g.getLength(),QRUtil.getLengthInBits(g.mode,a));g.write(d)}for(e=a=0;e<b.length;e++)a+=b[e].dataCount;if(d.getLengthInBits()>8*a)throw Error("code length overflow. ("+d.getLengthInBits()+">"+8*a+")");for(d.getLengthInBits()+4<=8*a&&d.put(0,4);0!=d.getLengthInBits()%8;)d.putBit(!1);for(;!(d.getLengthInBits()>=8*a);){d.put(QRCode.PAD0,8);if(d.getLengthInBits()>=
8*a)break;d.put(QRCode.PAD1,8)}return QRCode.createBytes(d,b)};
QRCode.createBytes=function(a,b){for(var c=0,d=0,e=0,g=Array(b.length),k=Array(b.length),f=0;f<b.length;f++){var l=b[f].dataCount,m=b[f].totalCount-l;d=Math.max(d,l);e=Math.max(e,m);g[f]=Array(l);for(var h=0;h<g[f].length;h++)g[f][h]=255&a.buffer[h+c];c+=l;h=QRUtil.getErrorCorrectPolynomial(m);l=(new QRPolynomial(g[f],h.getLength()-1)).mod(h);k[f]=Array(h.getLength()-1);for(h=0;h<k[f].length;h++)m=h+l.getLength()-k[f].length,k[f][h]=0<=m?l.get(m):0}for(h=f=0;h<b.length;h++)f+=b[h].totalCount;a=Array(f);
for(h=c=0;h<d;h++)for(f=0;f<b.length;f++)h<g[f].length&&(a[c++]=g[f][h]);for(h=0;h<e;h++)for(f=0;f<b.length;f++)h<k[f].length&&(a[c++]=k[f][h]);return a};
...
case QRMode.MODE_ALPHA_NUM:return 13;case QRMode.MODE_8BIT_BYTE:return 16;case QRMode.MODE_KANJI:return 12;default:throw Error("mode:"+a);}else throw Error("type:"+b);},getLostPoint:function(a){for(var b=a.getModuleCount(),c=0,d=0;d<b;d++)for(var e=0;e<b;e++){for(var g=0,k=a.isDark(d,e),f=-1;1>=f;f++)if(!(0>d+f||b<=d+f))for(var l=-1;1>=l;l++)0>e+l||b<=e+l||(0!=f||0!=l)&&k==a.isDark(d+f,e+l)&&g++;5<g&&(c+=3+g-5)}for(d=0;d<b-1;d++)for(e=0;e<b-1;e++)if(g=0,a.isDark(d,e)&&g++,a.isDark(d+1,e)&&g++,a.isDark(d,
e+1)&&g++,a.isDark(d+1,e+1)&&g++,0==g||4==g)c+=3;for(d=0;d<b;d++)for(e=0;e<b-6;e++)a.isDark(d,e)&&!a.isDark(d,e+1)&&a.isDark(d,e+2)&&a.isDark(d,e+3)&&a.isDark(d,e+4)&&!a.isDark(d,e+5)&&a.isDark(d,e+6)&&(c+=40);for(e=0;e<b;e++)for(d=0;d<b-6;d++)a.isDark(d,e)&&!a.isDark(d+1,e)&&a.isDark(d+2,e)&&a.isDark(d+3,e)&&a.isDark(d+4,e)&&!a.isDark(d+5,e)&&a.isDark(d+6,e)&&(c+=40);for(e=g=0;e<b;e++)for(d=0;d<b;d++)a.isDark(d,e)&&g++;return c+Math.abs(100*g/b/b-50)/5*10}},QRMath={glog:function(a){if(1>a)throw Error("glog("+
...
QRRSBlock.getRSBlocks=function(a,b){var c=QRRSBlock.getRsBlockTable(a,b);if(void 0==c)throw Error("bad rs block @ typeNumber:"+a+"/errorCorrectLevel:"+b);a=c.length/3;b=[];for(var d=0;d<a;d++)for(var e=c[3*d],g=c[3*d+1],k=c[3*d+2],f=0;f<e;f++)b.push(new QRRSBlock(g,k));return b};
