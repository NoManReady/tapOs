var template = {
	sealTpl:'<div class="kg-dialog kg-dialog-runSignature" id="sealTpl-dialog"><div class=" kg-switcher" id="kg-switcher" >'+
		...
		'<div class="kg-form">'+
			'<div class="form-item clearfix i-password">'+
				...
				'<div class="kg-sm kg-sm-6">'+
					'<input type="password" name="kgpassword" autocomplete="new-password" id="kg-password" class="form-control">'+
				'</div>'+
			'</div>'+
		'</div>'+
	'</div>'
...
var Signature = {
     ...
     * 解析签章对象成标准base64图片数据
     */
	toBase64Img: function(seal){// 印章数据转换成标准base64图片数据
		...
		if(keyData.result && keyData.seals && keyData.seals.length){
			...
			var d = dialog({
	            ...
	            ok:function(){
	            	...
	            	callback(JSON.stringify(result), keyData.keysn, keyData.orgname).then(function(ret){
