 ...
 * For details, see: http://www.gnu.org/licenses/lgpl-2.1.html
 */
;c("dialog",function(f){var i=f("jquery");var q=f("popup");var g=f("dialog-config");var j=g.cssUri;if(j){var l=f[f.toUrl?"toUrl":"resolve"];if(l){j=l(j);j='<link rel="stylesheet" href="'+j+'" />';if(i("base")[0]){i("base").before(j)}else{i("head").append(j)}}}var m=0;var p=new Date()-0;var k=!("minWidth" in i("html")[0].style);var o="createTouch" in document&&!("onmousemove" in document)||/(iPhone|iPad|iPod)/i.test(navigator.userAgent);var h=!k&&!o;var e=function(r,s,u){var w=r=r||{};if(typeof r==="string"||r.nodeType===1){r={content:r,fixed:!o}}r=i.extend(true,{},e.defaults,r);r.original=w;var v=r.id=r.id||p+m;var t=e.get(v);if(t){return t.focus()}if(!h){r.fixed=false}if(r.quickClose){r.modal=true;r.backdropOpacity=0}if(!i.isArray(r.button)){r.button=[]}if(u!==undefined){r.cancel=u}if(r.cancel){r.button.push({id:"cancel",value:r.cancelValue,callback:r.cancel,display:r.cancelDisplay})}if(s!==undefined){r.ok=s}if(r.ok){r.button.push({id:"ok",value:r.okValue,callback:r.ok,autofocus:true})}return e.list[v]=new e.create(r)};var d=function(){};d.prototype=q.prototype;var n=e.prototype=new d();e.create=function(s){var t=this;i.extend(this,new q());var v=s.original;var u=i(this.node).html(s.innerHTML);var r=i(this.backdrop);this.options=s;this._popup=u;i.each(s,function(w,x){if(typeof t[w]==="function"){t[w](x)}else{t[w]=x}});if(s.zIndex){q.zIndex=s.zIndex}u.attr({"aria-labelledby":this._$("title").attr("id","title:"+this.id).attr("id"),"aria-describedby":this._$("content").attr("id","content:"+this.id).attr("id")});this._$("close").css("display",this.cancel===false?"none":"").attr("title",this.cancelValue).on("click",function(w){t._trigger("cancel");w.preventDefault()});this._$("dialog").addClass(this.skin);this._$("body").css("padding",this.padding);if(s.quickClose){r.on("onmousedown" in document?"mousedown":"click",function(){t._trigger("cancel");return false})}this.addEventListener("show",function(){r.css({opacity:0,background:s.backdropBackground}).animate({opacity:s.backdropOpacity},150)});this._esc=function(y){var A=y.target;var B=A.nodeName;var w=/^input|textarea$/i;var x=q.current===t;var z=y.keyCode;if(!x||w.test(B)&&A.type!=="button"){return}if(z===27){t._trigger("cancel")}};i(document).on("keydown",this._esc);this.addEventListener("remove",function(){i(document).off("keydown",this._esc);delete e.list[this.id]});m++;e.oncreate(this);return this};e.create.prototype=n;i.extend(n,{content:function(s){var r=this._$("content");if(typeof s==="object"){s=i(s);r.empty("").append(s.show());this.addEventListener("beforeremove",function(){i("body").append(s.hide())})}else{r.html(s)}return this.reset()},title:function(r){this._$("title").text(r);this._$("header")[r?"show":"hide"]();return this},width:function(r){this._$("content").css("width",r);return this.reset()},height:function(r){this._$("content").css("height",r);return this.reset()},button:function(r){r=r||[];var u=this;var s="";var t=0;this.callbacks={};if(typeof r==="string"){s=r;t++}else{i.each(r,function(v,x){var y=x.id=x.id||x.value;var w="";u.callbacks[y]=x.callback;if(x.display===false){w=' style="display:none"'}else{t++}s+='<button type="button" i-id="'+y+'"'+w+(x.disabled?" disabled":"")+(x.autofocus?' autofocus class="ui-dialog-autofocus"':"")+">"+x.value+"</button>";u._$("button").on("click","[i-id="+y+"]",function(z){var A=i(this);if(!A.attr("disabled")){u._trigger(y)}z.preventDefault()})})}this._$("button").html(s);this._$("footer")[t?"show":"hide"]();return this},statusbar:function(r){this._$("statusbar").html(r)[r?"show":"hide"]();return this},_$:function(r){return this._popup.find("[i="+r+"]")},_trigger:function(s){var r=this.callbacks[s];return typeof r!=="function"||r.call(this)!==false?this.close().remove():this}});e.oncreate=i.noop;e.getCurrent=function(){return q.current};e.get=function(r){return r===undefined?e.list:e.list[r]};e.list={};e.defaults=g;return e});window.dialog=a("dialog")})();